...
module NaiveKnn : TacticianOnlineLearnerType = functor (TS : TacticianStructures) -> struct
  ...
  open LH
    type db_entry =
        { features : (feat_kind * feature) list;
          obj      : tactic
        }
    type database =
    ...
    let predict db f =
      ...
      let tdidfs = List.map
          (fun ent -> let x = tfidf db.length db.frequencies feats (remove_feat_kind ent.features) in (x, ent.obj))
          db.entries in
