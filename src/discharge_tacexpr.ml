  ...
  let rec evar_to_hole c = match DAst.get c with
    | Glob_term.GEvar (id, _) ->
      ...
      with Not_found ->
        ...
        DAst.make (Glob_term.GHole (snd @@ Evd.evar_source (Evd.find evd' (Evd.evar_key id.v evd')),
      ...
      (* if (not @@ Cset.is_empty @@ Cset.inter cook_cs cs ||
        ...
        if not @@ Id.Set.disjoint bound ids then
          ...
          try
            ...
            let t = (pids, (r, None), Patternops.pattern_of_constr env evd' (EConstr.of_constr typed')) in
            return t
          with
          | CErrors.Timeout as e ->
            raise e
