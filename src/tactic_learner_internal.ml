...
let evars_of_term sigma acc c =
  let rec evrec acc c =
    ...
    | Evar (n, l) -> Evar.Map.add n l (Array.fold_left evrec acc @@ Array.of_list l)
    ...
    if Evar.Map.mem e acc then acc else
      ...
      let evars = Evar.Map.mapi (fun e substs ->
          update_hyps_origin sigma hyps_origin substs
            (Evd.evar_filtered_context @@ Evd.find_undefined sigma e)) (Evar.Map.map Array.of_list evars) in
      Evar.Map.fold aux
        evars
...
type data_in = { outcomes : TS.outcome list; tactic : TS.tactic ; name : Constant.t; status : data_status; path : Libnames.full_path; sec_vars : Names.Id.Set.t }
