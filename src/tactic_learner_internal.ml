...
module TS = struct
  ...
  let proof_state_sigma ((map, ustate, _) : proof_state) =
    Evar.Map.fold (fun e { hyps; goal; _} evd -> 
        (* ~relevance:(EConstr.ERelevance.relevant) introduced in 9.1 *)
...
let evars_of_filtered_evar_info sigma acc evi =
    ...
    (* let acc = match Evd.evar_body evi with
     | Evar_empty -> acc
     (* | Evar_defined b -> evars_of_term sigma acc b  *)
    in *)
...
let goal_to_proof_state ps =
  ...
  ctx, Evd.ustate sigma, Evar.Map.find e ctx
...
let make_result before term sigma pss =
  ...
  term, evars, Evd.ustate sigma, List.map (fun ps -> Evar.Map.find (Goal.goal ps) evars) pss
