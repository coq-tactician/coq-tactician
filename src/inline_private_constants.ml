...
let inline env extra_ctx extra_deps { outcomes; tactic; name; status; path; sec_vars } =
  let rec inline_constr sigma extra_substs_map c = match Constr.kind c with
    ...
    | Evar (ev, substs) ->
      ...
      let new_args = (List.append extra_substs (Evd.expand_existential0 sigma (ev, substs))) in
      Constr.mkEvar (ev, SList.of_full_list new_args)
      (* Constr.mkEvar (ev, List.append extra_substs substs) *)
    | _ -> Constr.map (inline_constr sigma extra_substs_map) c in
  let single_proof_state_calc_changes { hyps; _ } =
  ...
  let inline_map sigma map =
    ...
    substs, Evar.Map.map (fun (ps, (_, extra_ctx)) -> inline_single_proof_state sigma (Evar.Map.map Array.to_list substs) extra_ctx ps) changes in
  let inline_proof_state ((map, ustate, { evar; _ }) as ps) =
    let sigma = proof_state_sigma ps in
    let _, map = inline_map sigma map in
  ...
  and inline_proof_step { executions; tactic } =
    ...
    ; tactic = tactic } in
  let inline_result ((term, map, ustate, pss) as tr) =
    let sigma = tactic_result_sigma tr in
    let substs, map = inline_map sigma map in
    inline_constr sigma (Evar.Map.map Array.to_list substs) term, map, ustate, List.map (fun { evar; _ } -> Evar.Map.find evar map) pss in
  ...
  if sideff = Safe_typing.empty_private_constants then t else
    ...
    let extra_ctx = List.map (fun (c, _) ->
      ...
      let extra_deps = List.map (fun pt -> get_id pt, fold_constr (fun c ids -> collect_vars ids c) pt Id.Set.empty)
          extra_ctx in
    List.map (inline env extra_ctx extra_deps) t