...
module NaiveKnnSubst (SF : sig type second_feat end) = functor (TS : TacticianStructures) -> struct
  ...
  open SF
    ...
    let learn db _loc outcomes tac ps_to_feat ctx_to_feat =
...
module SimpleNaiveSubstKnn : TacticianOnlineLearnerType = functor (TS : TacticianStructures) -> struct
  ...
  module FH = F(TS)
  open FH
  let learn db _loc outcomes tac = learn db _loc outcomes tac proof_state_to_simple_ints context_simple_ints
