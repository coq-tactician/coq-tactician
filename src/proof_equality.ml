...
let evars_equal evd1 evd2 (equal : (Evar.t * Evar.t) list) =
  ...
  and eq_evar_info ei1 ei2 =
    ...
    eq_constr_univs_test ~goal_spine:true
      (Namegen.rename_bound_vars_as_displayed env evd1 Names.Id.Set.empty [] (Evd.evar_concl ei1))
      (Namegen.rename_bound_vars_as_displayed env evd2 Names.Id.Set.empty [] (Evd.evar_concl ei2)) &&
...
let third_if_not_equal_command ~pstate t1 t2 t3 =
  let run t =
    ...
    ComTactic.solve ~pstate (Goal_select.get_default_goal_selector ()) ~info:None (id_interp t) ~with_end_tac:false in
  try
    let p1 = run t1 in
    let p2 = run t2 in
