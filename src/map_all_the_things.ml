...
module MakeMapper (M: MapDef) = struct
  ...
  let rec cases_pattern_expr_r_id_map f (case : cases_pattern_expr_r) =
    ...
    | CPatRecord xs ->
      ...
      CPatRecord xs
    (* depth introduced in 8.19. Right now we do nothing with it here. *)
    | CPatDelimiters (depth, d, c) ->
      CPatDelimiters (depth, d, cases_pattern_expr_id_map c)
    | CPatCast (c1, c2) ->
  ...
  let local_binder_expr_id_map f : local_binder_expr -> local_binder_expr = function
    | CLocalAssum (lis, r, bk, c) ->
      CLocalAssum (OList.map ((CAst.map (name_id_map f))) lis, r, bk, c)
    | CLocalDef (li, r, c1, c2) ->
      CLocalDef (CAst.map (name_id_map f) li, r, c1, c2)
    | CLocalPattern pat ->
      (* 8.13 changes: pat was (cases_pattern_expr * constr_expr option) CAst.t
         but now it is just cases_pattern_expr
         see: https://ocaml.org/p/coq/8.13.0/doc/Constrexpr/index.html#type-local_binder_expr *)
      (* CLocalPattern (CAst.map (fun (ca, c) -> cases_pattern_expr_id_map f ca, c) pat) *)
      CLocalPattern (cases_pattern_expr_id_map f pat)
  ...
  let rec glob_constr_r_map m r c' =
    ...
    m.glob_constr c' @@ function
     ...
     | GLambda (id, r, bk, typ, term) ->
       ...
       GLambda (name_id_map id_f id, r, bk, typ, term)
     | GProd (id, r, bk, typ, term) ->
       let+ typ = glob_constr_map typ
       and+ id_f, term = with_binders (filter_placeholders [id]) term glob_constr_map in
       GProd (name_id_map id_f id, r, bk, typ, term)
     | GLetIn (id, r, b, typ, term) ->
       let+ b = glob_constr_map b
       and+ typ = option_map glob_constr_map typ
       and+ id_f, term = with_binders (filter_placeholders [id]) term glob_constr_map in
       GLetIn (name_id_map id_f id, r, b, typ, term)
     ...
     | GRec (fk, ids, decls, typs, terms) ->
       ...
       and+ terms = array_map (fun (dec, (t, decls)) ->
           let bndrs = filter_placeholders @@ OList.map (fun (id, _, _, _, _) -> id) decls in
           let* ids_f, t = with_binders bndrs t glob_constr_map in
           let+ decls = List.map (fun (id, r, bk, c1, c2) ->
               let+ c1 = option_map glob_constr_map c1
               and+ c2 = glob_constr_map c2 in
               name_id_map ids_f id, r, bk, c1, c2) decls in
           ids_f dec, (t, decls))
