...
let discharge_outcomes senv { outcomes; tactic; name; status; path } =
  try
    ...
    let info =
      ...
      let name = if Str.string_match (Str.regexp "^\\(.*\\)_obligation_[0-9]*$") name_str 0 then
        let obligation_parent = Names.Constant.change_label name @@
          Names.Label.make @@ Str.matched_group 1 name_str in
        if Environ.mem_constant obligation_parent @@ Safe_typing.env_of_safe_env senv then
          obligation_parent
        else name
      else
        name in
      Section.segment_of_constant name @@ Option.get sections in
...
let tclDebugTac t env debug =
    ...
    tclPROGRESS @@ tclTIMEOUTF 0.1 @@ parse_tac t
