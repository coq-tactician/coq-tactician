...
let discharge_outcomes { outcomes; tactic; name; status; path } =
  let modlist = Lib.replacement_context () in
  ...
  let calc_defined_deps sigma es =
    ...
    Evar.Set.fold
      (fun e evs -> match (find sigma e).evar_body with
         ...
         | Evar_defined term -> Evar.Set.union evs @@ Evd.evars_of_term @@
           EConstr.to_constr ~abort_on_undefined_evars:false sigma term)
  ...
  | Some (db, exn, sideff) ->
    ...
    (List.iter add @@ List.rev db; Hashtbl.remove int64_to_knn id;
     ...
     | Some exn ->
       ...
       if not !Flags.quiet || !Vernacentries.test_mode then begin
    ...
    let tryadd (execs, tac) =
      let filter =
        try
          ...
          String.equal s "admit" || String.equal s "synth" || string_is_prefix "synth with cache" s
          || string_is_prefix "tactician ignore" s || string_is_prefix "fix" s || string_is_prefix "cofix" s
          || string_is_prefix "change_no_check" s || string_is_prefix "exact_no_checK" s
          || string_is_prefix "native_cast_no_check" s || string_is_prefix "vm_cast_no_check" s
          || string_is_prefix "shelve" s
        with _ -> false in
