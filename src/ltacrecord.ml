...
let recorder (tac : glob_tactic_expr) id name : unit Proofview.tactic = (* TODO: Implement self-learning *)
  ...
  let save_db env sideff (db : localdb) =
    ...
    let tryadd (execs, tac) =
      ...
      if (String.equal s "admit" || String.equal s "synth" || string_is_prefix "synth with cache" s
          || string_is_prefix "tactician ignore" s || string_is_prefix "fix" s || string_is_prefix "cofix" s
          || string_is_prefix "change_no_check" s || string_is_prefix "exact_no_checK" s || string_is_prefix "native_cast_no_check" s
          || string_is_prefix "vm_cast_no_check" s)
